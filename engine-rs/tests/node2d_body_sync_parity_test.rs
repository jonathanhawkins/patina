...
// ===========================================================================
// 12. Gravity scale registration and effect on simulation
// ===========================================================================

#[test]
fn gravity_scale_zero_disables_gravity_for_rigid_body() {
    let mut tree = SceneTree::new();
    let root = tree.root_id();
    ...
    let mut server = PhysicsServer::new();
    server.set_gravity(Vector2::new(0.0, 980.0));
    server.register_bodies(&tree);

    let body_id = server.body_for_node(rigid_id).unwrap();
    assert_eq!(
        server.world().get_body(body_id).unwrap().gravity_scale,
        0.0,
        "gravity_scale should be read from node property"
    );

    // Step several frames — body should not fall
    for _ in 0..10 {
        server.step_physics(1.0 / 60.0);
    }
    server.sync_from_physics(&mut tree);
    ...
    let pos = get_vec2(&tree, rigid_id, "position");
    assert!(
        (pos.y - 100.0).abs() < 0.01,
        "zero gravity_scale body should not fall, got y={}",
        pos.y
    );
}

#[test]
fn gravity_scale_negative_inverts_gravity() {
    let mut tree = SceneTree::new();
    let root = tree.root_id();

    let mut node = Node::new("Riser", "RigidBody2D");
    node.set_property("position", Variant::Vector2(Vector2::new(100.0, 500.0)));
    ...
    assert!(
        pos.y < 500.0,
        "negative gravity_scale should make body rise, got y={}",
        pos.y
    );
}
...
// ===========================================================================
// 18. Gravity affects rigid but not kinematic/static
// ===========================================================================

#[test]
fn gravity_only_affects_rigid_bodies() {
    let mut tree = SceneTree::new();
    let root = tree.root_id();
    ...
    let rigid_pos = get_vec2(&tree, rigid_id, "position");
    assert!(rigid_pos.y > 100.0, "rigid body should fall under gravity");

    // Static body unchanged
    let static_pos_after = get_vec2(&tree, static_id, "position");
    assert_eq!(static_pos_before, static_pos_after, "static unaffected by gravity");

    // Kinematic body: no velocity set, so position should not have moved
    let char_pos_after = get_vec2(&tree, char_id, "position");
    assert!(
        (char_pos_after.x - 200.0).abs() < 0.01,
        "kinematic body without velocity should stay put, got x={}",
        char_pos_after.x
    );
    let char_body_id = server.body_for_node(char_id).unwrap();
    let char_body = server.world().get_body(char_body_id).unwrap();
    assert_eq!(char_body.body_type, BodyType::Kinematic);
}
