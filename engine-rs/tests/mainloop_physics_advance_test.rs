...
/// Setting gravity to zero on the PhysicsServer stops all gravitational motion.
#[test]
fn zero_gravity_world_no_falling() {
    let (mut ml, rigid_id) = make_gravity_test();
    ml.physics_server_mut().set_gravity(Vector2::ZERO);

    let initial = get_pos(&ml, rigid_id);
    ml.run_frames(30, 1.0 / 60.0);
    let final_pos = get_pos(&ml, rigid_id);

    assert!(
        approx_eq_f32(initial.x, final_pos.x) && approx_eq_f32(initial.y, final_pos.y),
        "zero-gravity world should not move body: {:?} -> {:?}",
        initial,
        final_pos
    );
}

/// PhysicsServer gravity getter returns what was set.
#[test]
fn gravity_getter_returns_set_value() {
    let (mut ml, _) = make_gravity_test();
    let custom = Vector2::new(0.0, 500.0);
    ml.physics_server_mut().set_gravity(custom);

    let g = ml.physics_server().gravity();
    assert!(
        approx_eq_f32(g.x, custom.x) && approx_eq_f32(g.y, custom.y),
        "gravity getter should return set value: {:?} vs {:?}",
        g,
        custom
    );
}
...
// ===========================================================================
// 4. Custom gravity vector (sideways)
// ===========================================================================

/// Sideways gravity makes bodies drift horizontally instead of vertically.
#[test]
fn sideways_gravity_causes_horizontal_drift() {
    let (mut ml, rigid_id) = make_gravity_test();
    // Gravity pointing right
    ml.physics_server_mut()
        .set_gravity(Vector2::new(500.0, 0.0));
    ...
    assert!(
        final_pos.x > initial.x + 10.0,
        "sideways gravity should push body right: {} > {}",
        final_pos.x,
        initial.x + 10.0
    );
    assert!(
        (final_pos.y - initial.y).abs() < 1.0,
        "Y should not change significantly with horizontal gravity: delta={}",
        (final_pos.y - initial.y).abs()
    );
}

// ===========================================================================
// 5. Force application through PhysicsServer → MainLoop
// ===========================================================================

/// Applying a force to a body through PhysicsServer accelerates it.
#[test]
fn force_application_through_mainloop() {
    let (mut ml, rigid_id) = make_gravity_test();
    // Zero gravity so we isolate force effect
    ml.physics_server_mut().set_gravity(Vector2::ZERO);
    ...
    ml.physics_server_mut()
        .apply_force_to_node(rigid_id, Vector2::new(1000.0, 0.0));

    // Run a few frames — force is consumed on next step
    ml.run_frames(10, 1.0 / 60.0);

    let final_pos = get_pos(&ml, rigid_id);
    assert!(
        final_pos.x > initial.x,
        "applied force should move body right: {} > {}",
        final_pos.x,
        initial.x
    );
}

/// Force only affects rigid bodies, not static bodies.
#[test]
fn force_does_not_affect_static_body() {
    let mut tree = SceneTree::new();
    let root = tree.root_id();
    ...
    let mut shape = Node::new("Shape", "CollisionShape2D");
    shape.set_property("size", Variant::Vector2(Vector2::new(100.0, 10.0)));
    tree.add_child(floor_id, shape).unwrap();

    let mut ml = MainLoop::new(tree);
    ml.register_physics_bodies();

    let initial = get_pos(&ml, floor_id);
    ml.physics_server_mut()
        .apply_force_to_node(floor_id, Vector2::new(10000.0, 0.0));
    ml.run_frames(10, 1.0 / 60.0);
    let final_pos = get_pos(&ml, floor_id);

    assert!(
        approx_eq_f32(initial.x, final_pos.x) && approx_eq_f32(initial.y, final_pos.y),
        "static body should not move under force: {:?} -> {:?}",
        initial,
        final_pos
    );
}
...
// ===========================================================================
// 6. Impulse application through PhysicsServer → MainLoop
// ===========================================================================

/// Applying an impulse instantly changes velocity.
#[test]
fn impulse_instantly_changes_velocity() {
    let (mut ml, rigid_id) = make_gravity_test();
    ml.physics_server_mut().set_gravity(Vector2::ZERO);

    // Apply upward impulse
    ml.physics_server_mut()
        .apply_impulse_to_node(rigid_id, Vector2::new(0.0, -100.0));
    ...
    assert!(
        vel.y < -50.0,
        "impulse should create upward velocity: got {}",
        vel.y
    );
}

/// Multiple impulses accumulate.
#[test]
fn multiple_impulses_accumulate() {
    let (mut ml, rigid_id) = make_gravity_test();
    ml.physics_server_mut().set_gravity(Vector2::ZERO);

    // Apply two rightward impulses
    ml.physics_server_mut()
        .apply_impulse_to_node(rigid_id, Vector2::new(50.0, 0.0));
    ml.physics_server_mut()
        .apply_impulse_to_node(rigid_id, Vector2::new(50.0, 0.0));
    ...
    assert!(
        vel.x > 80.0,
        "two impulses of 50 should give ~100 velocity: got {}",
        vel.x
    );
}
