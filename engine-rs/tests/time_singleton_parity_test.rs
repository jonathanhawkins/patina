...
fn get_ticks_usec_callable() {
    ...
    assert!(t >= 1000, "should advance at least 1000us after 2ms sleep, got {t}");
}
...
fn get_ticks_usec_microsecond_resolution() {
    ...
    let b = os::get_ticks_usec();
    let diff = b - a;
    assert!(diff >= 1000, "5ms sleep should show >= 1000us difference, got {diff}");
}
...
fn get_ticks_msec_consistent_with_usec() {
    ...
    let diff = if us_as_ms > ms { us_as_ms - ms } else { ms - us_as_ms };
    assert!(diff < 50, "usec/1000 and msec should be within 50ms, diff={diff}");
}
