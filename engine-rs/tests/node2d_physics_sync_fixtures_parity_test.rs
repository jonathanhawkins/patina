...
// ===========================================================================
// 19. Area2D position syncs from scene node via sync_to_physics
// ===========================================================================

#[test]
fn area2d_position_syncs_from_node_on_sync_to_physics() {
    let mut tree = SceneTree::new();
    let root = tree.root_id();
    LifecycleManager::enter_tree(&mut tree, root);

    let mut area_node = Node::new("Zone", "Area2D");
    area_node.set_property("position", Variant::Vector2(Vector2::new(100.0, 200.0)));
    let area_id = tree.add_child(root, area_node).unwrap();
    ...
    let phys_area_id = server.area_for_node(area_id).expect("area registered");
    let area = server.area_store().get_area(phys_area_id).unwrap();
    assert!(
        approx_vec2(area.position, Vector2::new(100.0, 200.0)),
        "initial area position should match node: {:?}",
        area.position
    );
    ...
    // Sync should update the area position.
    server.sync_to_physics(&tree);

    let area = server.area_store().get_area(phys_area_id).unwrap();
    assert!(
        approx_vec2(area.position, Vector2::new(500.0, 300.0)),
        "area position should follow node after sync_to_physics: {:?}",
        area.position
    );
}
...
// ===========================================================================
// 26. Gravity scale read from node property on registration
// ===========================================================================

#[test]
fn gravity_scale_preserved_on_registration() {
    let mut tree = SceneTree::new();
    let root = tree.root_id();
    LifecycleManager::enter_tree(&mut tree, root);
    ...
    assert!(
        (body.gravity_scale - 0.0).abs() < 1e-6,
        "gravity_scale should be 0.0, got {}",
        body.gravity_scale
    );
}
...
fn gravity_scale_default_is_one() {
    ...
    // No explicit gravity_scale property — should default to 1.0.
    let rigid = add_body_node(
        &mut tree, root, "Default", "RigidBody2D",
        Vector2::ZERO, 10.0,
    );
    ...
    let body_id = server.body_for_node(rigid).expect("body registered");
    let body = server.world().get_body(body_id).expect("body exists");

    assert!(
        (body.gravity_scale - 1.0).abs() < 1e-6,
        "default gravity_scale should be 1.0, got {}",
        body.gravity_scale
    );
}

// ===========================================================================
// 27. Area2D position tracks node mutation across frames
// ===========================================================================

#[test]
fn area2d_position_tracks_node_across_frames() {
    let mut tree = SceneTree::new();
    let root = tree.root_id();
    LifecycleManager::enter_tree(&mut tree, root);

    let mut area_node = Node::new("Zone", "Area2D");
    area_node.set_property("position", Variant::Vector2(Vector2::new(10.0, 20.0)));
    let area_nid = tree.add_child(root, area_node).unwrap();

    let mut shape = Node::new("Shape", "CollisionShape2D");
    shape.set_property("radius", Variant::Float(15.0));
    tree.add_child(area_nid, shape).unwrap();

    let mut server = PhysicsServer::new();
    server.register_bodies(&tree);

    let phys_area_id = server.area_for_node(area_nid).expect("area registered");

    // Move area across multiple frames.
    let positions = [
        Vector2::new(50.0, 60.0),
        Vector2::new(100.0, 120.0),
        Vector2::new(200.0, 240.0),
    ];
    for (i, &pos) in positions.iter().enumerate() {
        tree.get_node_mut(area_nid)
            .unwrap()
        ...
        let area = server.area_store().get_area(phys_area_id).unwrap();
        assert!(
            approx_vec2(area.position, pos),
            "frame {i}: area position should be {pos:?}, got {:?}",
            area.position
        );
    }
}
...
// ===========================================================================
// 28. Negative gravity_scale inverts gravity direction
// ===========================================================================

#[test]
fn negative_gravity_scale_preserved_on_registration() {
    let mut tree = SceneTree::new();
    let root = tree.root_id();
    LifecycleManager::enter_tree(&mut tree, root);

    let mut node = Node::new("AntiGrav", "RigidBody2D");
    node.set_property("position", Variant::Vector2(Vector2::ZERO));
    ...
    assert!(
        (body.gravity_scale - (-1.0)).abs() < 1e-6,
        "gravity_scale should be -1.0, got {}",
        body.gravity_scale
    );
}
