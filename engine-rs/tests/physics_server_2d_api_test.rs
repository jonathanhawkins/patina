...
fn body_get_state_position() {
    let mut world = PhysicsWorld2D::new();
    let body = PhysicsBody2D::new(
        BodyId(0), BodyType::Rigid, Vector2::new(10.0, 20.0),
...
fn body_types_static_kinematic_rigid() {
    ...
    let s = world.add_body(PhysicsBody2D::new(BodyId(0), BodyType::Static, Vector2::ZERO, shape, 1.0));
    let k = world.add_body(PhysicsBody2D::new(BodyId(0), BodyType::Kinematic, Vector2::new(5.0, 0.0), shape, 1.0));
    let r = world.add_body(PhysicsBody2D::new(BodyId(0), BodyType::Rigid, Vector2::new(10.0, 0.0), shape, 1.0));
...
fn body_count_tracks_additions() {
    ...
    let shape = Shape2D::Circle { radius: 1.0 };
    world.add_body(PhysicsBody2D::new(BodyId(0), BodyType::Rigid, Vector2::ZERO, shape, 1.0));
    assert_eq!(world.body_count(), 1);

    world.add_body(PhysicsBody2D::new(BodyId(0), BodyType::Static, Vector2::ZERO, shape, 1.0));
...
fn step_advances_rigid_body() {
    ...
    let b = world.get_body(id).unwrap();
    assert!((b.position.x - 60.0).abs() < 1.0, "should move ~60 units in 1s at v=60");
}
